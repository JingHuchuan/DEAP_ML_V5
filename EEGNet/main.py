...
if __name__ == '__main__':
    ...
    for i in range(args.nUser):
        ...
        # 注意这里都要减1，下标从0开始
        # channel_selection = [4, 3, 0, 25, 7, 2, 16, 20, 21, 11, 1, 29, 19, 8, 6, 17, 26, 9, 15, 30, 27, 24, 13, 31, 22,
        #                      28, 5, 23, 18, 12, 10, 14]

        # channel_selection = [1, 3, 5, 4, 0, 6, 10, 12, 9, 11, 2, 8, 13, 7]  # DREAMER的通道
        #
        # data = data[:, channel_selection[:1], :]

        # channel_selection = [20, 17, 1, 4, 0, 6, 29, 11]  # DEAP数据集的通道选取
        # channel_selection = [20, 17, 1, 4, 0]  # DEAP数据集的通道选取
        # data = data[:, channel_selection, :]

        # channel_selection = [10, 4, 7, 1, 6, 3, 2, 5]  # DREAMER数据集的通道选取
        # data = data[:, channel_selection, :]

        label = np.load(args.label_path + 's{}.npy'.format(i + 1))[:, 0]  # 第一个维度
        ...
        net = SelfNet(args.routing_iterations, args.num_classes)  # 二分类
        net = net.to(device)
